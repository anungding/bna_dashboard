...
def load_csv(file_path):
    ...
    # Bersihkan kolom kecamatan
    df['kecamatan'] = df['kecamatan'].str.replace(" ", "") 
    df['kecamatan'] = df['kecamatan'].str.replace("PurwarejaKlampok", "Purworeja Klampok")
    
    # Mengganti tanda '-' dengan 0 agar bisa dikonversi ke angka
    df.replace("-", 0, inplace=True)
    
    # Konversi semua kolom selain 'kecamatan' ke float
    for column in df.columns:
        if column != "kecamatan":
            df[column] = pd.to_numeric(df[column], errors="coerce").fillna(0)  # Jika tidak bisa dikonversi, ganti dengan 0
...
color_mapping = {
    "Penduduk-BPS-2019": "Reds_09",  # Skema merah untuk penduduk
    "Pertanian-Sawah-BPS-2023": "Greens_09"  # Skema hijau untuk pertanian
}
...
for feature in geojson_data['features']:
    ...
    if total_value.size > 0:
        feature['properties']['Total'] = float(total_value[0])  # Gunakan float agar bisa menangani desimal
    else:
...
selected_color = color_mapping.get(dataset_option, "Reds_09")  # Default merah
...
def get_color_based_on_data(feature):
    ...
    colormap = branca.colormap.linear.__getattribute__(selected_color).scale(min_total, max_total)  # Sesuaikan warna
